...
export const PlaygroundView: React.FC<PlaygroundViewProps> = ({
  ...
  const prefetchNextQuestion = async () => {
    try {
      const question = await getQuestion(query, levelRef.current, userContext);
  ...
  const handleAnswer = (index: number) => {
    ...
    if (isPaused) {
      toast.error("Need to resume the Playground to answer the questions");
      return;
    }
    // Change the diffcult level
    if (index === currentQuestion.correctAnswer && currentQuestionTime <= 5) {
